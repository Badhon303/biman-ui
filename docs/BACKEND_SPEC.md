...
- Raises **maintenance tickets** (work orders). PM tickets use Service F, B, C, D, E, or V:
  - F/B/C/D/E tickets are generated when an authorized user manually triggers an hour-meter service
    check and the equipment crosses the applicable service band.
  - V-Service tickets are generated by the six-month maintenance schedule.
  - Only Breakdown, General, and Washing tickets are manually created by users. `Others` is not a
    manually selectable ticket category; if retained for compatibility, it is a system/internal
    classification for hour-meter-band work. It will be created manually when an equipment type services are
    selected while creating equipment type.
...
- `Ticket.pmType` / `serviceType` is an enum matching the PM services `F-Service | B-Service |
  C-Service | D-Service | E-Service | V-Service`, or one of `Breakdown | General | Washing` for
  user-created tickets. If compatibility with the mock frontend requires `Others`, keep it as an
  internal/system-only value and never expose it in the manual-create DTO.
...
- **PM services** are exactly F-Service, B-Service, C-Service, D-Service, E-Service, and
  V-Service. F/B/C/D/E use `minHours` and `maxHours` service bands. Bands must be validated as
  non-overlapping and ordered for each equipment type.
- V-Service is the only scheduled service and uses `months = 6`. It is a recurring calendar
  service; F/B/C/D/E are never created by a calendar scheduler.
- Equipment types may have custom hour bands if the business needs them, but custom bands must
  still be PM service definitions and must not become manually creatable ticket categories.
...
- **Hour meter updates**: Engineer, Manager, or Super Admin can update an equipment's hour meter
  ...
  5. The manual service-check compares the previous serviced threshold/cycle with the current
     ...
     a. Create one PM ticket with `serviceType` equal to the applicable F/B/C/D/E service,
        `priority = Medium` by default, `status = Open`, and the authenticated user as actor.
  ...
  7. A manual service-check may be performed immediately after recording the meter or later. It
  ...
  8. Do not put hour-meter crossing in a cron job, `SchedulerModule`, or BullMQ recurring job.
...
- PM `serviceType` values are `F-Service | B-Service | C-Service | D-Service | E-Service |
  V-Service`.
  - F/B/C/D/E are hour-meter-band PM services. They are generated only by the explicit manual
    hour-meter service-check (§5.3), never by a scheduler and never from the ordinary manual ticket
    form.
...
- User-created ticket types are **only** `Breakdown`, `General`, and `Washing`, and these can be
  ...
  required.
- Do not expose `Others` as a manually selectable type. If the existing frontend enum retains
  `Others`, treat it as a compatibility/internal category for hour-meter-band work, not as an
  additional user-created ticket type.
...
1. **Checklist templates**: mock data has one shared 60-item checklist, but the requirement states
   ...
   `EquipmentType`, per PM service (`F/B/C/D/E/V`), or both. Recommendation: key templates by
   `(equipment_type_id, service_type)` with fallback to a global default.
