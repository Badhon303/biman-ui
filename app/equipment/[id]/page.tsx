...
export default function EquipmentProfile() {
  ...
  const save = () => {
    const parsedHourMeter = hourMeterInput.trim() ? Number(hourMeterInput) : undefined;
    if (parsedHourMeter !== undefined && !Number.isFinite(parsedHourMeter)) {
  ...
  const addSpecification = () => {
    setE((current) => ({
      ...current,
      specifications: [...current.specifications, { label: "New spec", value: "" }],
    }));
  };
    ...
    <ShellPage>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={() => toast.success("Print dialog opened (mock)")}>
          ...
          <div className="grid flex-1 gap-3 sm:grid-cols-3">
            ...
            <label className="text-xs font-semibold">
              ...
              <Input
                ...
                onChange={(ev) => updateDraft({ manufacturer: ev.target.value })}
              />
            </label>
            <label className="text-xs font-semibold">
              Model
              <Input
                className="mt-2"
                value={draft.model}
                onChange={(ev) => updateDraft({ model: ev.target.value })}
              />
            </label>
            <label className="text-xs font-semibold sm:col-span-2">
              ...
              <Input
                ...
                onChange={(ev) => updateDraft({ location: ev.target.value })}
              />
            </label>
            <label className="text-xs font-semibold">
              Status
              <Select
                ...
                value={draft.status}
                onChange={(ev) => updateDraft({ status: ev.target.value as EquipmentStatus })}
              >
                {statuses.map((s) => (
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[.7fr_1.3fr]">
        <Card className="self-start p-6">
          <UploadPhoto
            label="Primary image"
            className="h-56 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900"
          />
          ...
          <div className="flex gap-1 overflow-x-auto border-b px-4 pt-3">
            {tabs.map((item) => (
              <button
                key={item}
                onClick={() => setTab(item)}
                className={`whitespace-nowrap border-b-2 px-3 py-3 text-sm font-semibold ${tab === item ? "border-blue-600 text-blue-600" : "border-transparent text-slate-500 hover:text-slate-800"}`}
              >
                {item}
              </button>
            ))}
              ...
              (editing ? (
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="text-xs font-semibold">
                    ...
                    <Input
                      className="mt-2"
                      value={draft.bimanSerialNo}
                      onChange={(ev) => updateDraft({ bimanSerialNo: ev.target.value })}
                    />
                  </label>
                  <label className="text-xs font-semibold">
                    TLD serial number
                    <Input
                      className="mt-2"
                      value={draft.TLDSerialNo}
                      onChange={(ev) => updateDraft({ TLDSerialNo: ev.target.value })}
                    />
                  </label>
                  <label className="text-xs font-semibold">
                    Engine model
                    <Input
                      ...
                      onChange={(ev) => updateDraft({ engineModel: ev.target.value })}
                    />
                  </label>
                  <label className="text-xs font-semibold">
                    Engine S/L
                    <Input
                      className="mt-2"
                      value={draft.engineSerialNo}
                      onChange={(ev) => updateDraft({ engineSerialNo: ev.target.value })}
                    />
                  </label>
                  <label className="text-xs font-semibold">
                    Hour meter
                  ...
                  <label className="text-xs font-semibold">
                    ...
                    <Input
                      className="mt-2"
                      type="date"
                      value={draft.actualGTDate ?? ""}
                      onChange={(ev) => updateDraft({ actualGTDate: ev.target.value })}
                    />
                  </label>
                  <label className="text-xs font-semibold">
                    Ship date
                    <Input
                      ...
                      onChange={(ev) => updateDraft({ shipDate: ev.target.value })}
                    />
                  </label>
                  <label className="text-xs font-semibold">
                    Shipping status
                    <Input
                      ...
                      onChange={(ev) => updateDraft({ shippingStatus: ev.target.value })}
                    />
                  </label>
                  <label className="text-xs font-semibold">
                    Emission rating
                    <Input
                      className="mt-2"
                      value={draft.emissionRatting ?? ""}
                      onChange={(ev) => updateDraft({ emissionRatting: ev.target.value })}
                    />
                  </label>
                </div>
              ) : (
...
function UploadPhoto({
  ...
  return (
    <label
      className={`relative grid cursor-pointer place-items-center overflow-hidden border border-dashed bg-slate-50 text-slate-400 transition hover:border-blue-400 hover:bg-blue-50 hover:text-blue-600 dark:bg-slate-950 dark:hover:bg-blue-950/30 ${className}`}
    >
      <input
        type="file"
        accept="image/*"
      ...
      {preview ? (
        ...
        <span className="text-center">
          <span className="mx-auto grid h-7 w-7 place-items-center rounded-full border border-current text-lg leading-none">
            +
          </span>
          <span className="mt-2 block text-[10px] font-medium">{label}</span>
        </span>
      )}
    </label>
  );
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {specifications.map((specification) => (
          <div
            ...
            className="rounded-xl border bg-slate-50 p-4 dark:bg-slate-900/60"
          >
  ...
  return (
    <div className="space-y-3">
      {specifications.map((specification, index) => (
        <div
          key={index}
          className="flex items-center gap-2 rounded-xl border bg-slate-50 p-3 dark:bg-slate-900/60"
        >
          <Input
            className="flex-1"
            placeholder="Label"
...
function Logbook({ history }: Readonly<{ history: typeof tickets }>) {
  return history.length ? (
    <div className="space-y-4">
      {history.map((ticket) => (
        <div key={ticket.id} className="flex gap-3">
          <div className="mt-1 rounded-full bg-blue-50 p-2 text-blue-600 dark:bg-blue-950/40">
    ...
    <EmptyState label="No maintenance history" />
  );
}
