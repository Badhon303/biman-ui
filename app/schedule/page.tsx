...
import { CalendarClock, CheckCircle2, CircleAlert, Clock3, Plus, Search, Ticket as TicketIcon } from "lucide-react";
...
export default function SchedulePage() {
  ...
  const rows = useMemo(
    () =>
      maintenanceSchedules.filter((schedule) => {
        const matchesType = filter === "All" || schedule.type === filter;
        const effectiveStatus = generatedTickets[schedule.id] ? "Ticket generated" : schedule.status;
        const matchesStatus = status === "All" || effectiveStatus === status;
        const matchesSearch = `${schedule.scheduleNo} ${schedule.activity} ${equipmentName(schedule.equipmentId)} ${schedule.assignedEngineer ?? ""}`
          .toLowerCase()
          .includes(search.toLowerCase());
        return matchesType && matchesStatus && matchesSearch;
      }),
    [filter, generatedTickets, search, status],
  );
  ...
  const totalDue = maintenanceSchedules.filter((schedule) => schedule.status === "Overdue" || schedule.status === "Due soon").length;
  const activeCount = maintenanceSchedules.filter((schedule) => schedule.status === "In progress").length;
  const completedCount = maintenanceSchedules.filter((schedule) => schedule.status === "Completed").length;
  ...
  return (
    <ShellPage>
      <PageHeader
        ...
        action={
          <Button onClick={() => toast.info("Schedule creation is ready for configuration.")}>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <SummaryCard icon={<CalendarClock className="h-5 w-5" />} label="Total schedules" value={maintenanceSchedules.length} detail="PM and washing plans" />
        <SummaryCard icon={<CircleAlert className="h-5 w-5" />} label="Due attention" value={totalDue} detail="Due soon or overdue" tone="amber" />
        <SummaryCard icon={<Clock3 className="h-5 w-5" />} label="In progress" value={activeCount} detail="Currently being worked" tone="blue" />
        <SummaryCard icon={<CheckCircle2 className="h-5 w-5" />} label="Completed" value={completedCount} detail="Latest cycle completed" tone="emerald" />
      </div>
      ...
      <Card>
        <div className="flex flex-col gap-4 border-b p-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="relative max-w-sm flex-1">
            ...
            <Input className="pl-9" placeholder="Search schedule or equipment" value={search} onChange={(event) => setSearch(event.target.value)} />
          </div>
        ...
        <Table>
          ...
          <TBody>
            {rows.map((schedule) => {
              ...
              return (
                <TR key={schedule.id} className={schedule.status === "Overdue" ? "bg-rose-50/40 dark:bg-rose-950/10" : ""}>
                  <TD>
                    <div className="font-semibold text-slate-900 dark:text-white">{schedule.scheduleNo}</div>
                    <div className="mt-1 text-xs text-slate-400">{schedule.activity}</div>
                  </TD>
                  <TD>
                    <div className="font-medium text-slate-800 dark:text-slate-100">{equipmentName(schedule.equipmentId).split(" · ")[0]}</div>
                    <div className="text-xs text-slate-400">{equipmentName(schedule.equipmentId).split(" · ")[1]}</div>
                  </TD>
                  <TD>
                  ...
                  <TD>{formatDate(schedule.nextDate)}</TD>
                  <TD className={schedule.status === "Overdue" ? "font-semibold text-rose-600" : ""}>{formatDate(schedule.dueDate)}</TD>
                  <TD>{schedule.scheduleDuration}</TD>
                  <TD><StatusBadge status={displayStatus} /></TD>
                  <TD>
                    {hasTicket ? (
                      <Link href={schedule.ticketId ? `/tickets/${schedule.ticketId}` : "/tickets"} className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600">
                        <TicketIcon className="h-3.5 w-3.5" />
                        {ticketNo ?? "View ticket"}
                      </Link>
                    ) : (
                      <Button variant="outline" className="px-2.5 py-1.5 text-xs" onClick={() => generateTicket(schedule)}>
                        <TicketIcon className="h-3.5 w-3.5" />
                        Generate ticket
                      </Button>
                    )}
...
function SummaryCard({ icon, label, value, detail, tone = "slate" }: { icon: React.ReactNode; label: string; value: number; detail: string; tone?: "slate" | "amber" | "blue" | "emerald" }) {
  const tones = {
